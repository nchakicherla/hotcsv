...
objectAsChars(void* obj_in, size_t nested_level, bool oneline) {
    ...
    if(NULL == (output_seq = calloc(output_seq_size + 1, sizeof(char))) ) { 
        fatalException(FN, "Allocate failed for: output_seq (calloc)");
    }
    ...
    switch(TYPE_OF(obj_in)) {
        ...
        case FLT_TYPE: {
            ...
            if( ((flt*)obj_in)->val < 0.001) { //handle small numbers
                ...
                if(!(num_char_buf = calloc(NUM_CHAR_BUF_LEN, sizeof(char)))) {
                    free(output_seq);
                    fatalException(FN, "Allocate failed for: num_char_buf (calloc)");
                }
        ...
        default: { //assume char* as type
            ...
            if(temp_seq != NULL) {
                ...
                fatalException(FN, "Initialization failed for temp_seq (seqDupl)");
            }
        }
    }
