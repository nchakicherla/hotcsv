...
seqDupl(const char* seq_in) {
    ...
    if(!(seq_out = malloc(seq_in_len + 1))) {
        fatalException(FN, "Allocate failed for: seq_out (malloc)");
    }
...
setString(str* str_in, const char* seq_in) {
    ...
    if(!(str_in->seq = malloc(seqLen(seq_in) + 1))) {
        fatalException(FN, "Allocate failed for: str_in->seq (malloc)");
    }
...
stringInit(const char* seq_in) {
    ...
    if(!(output_str = malloc(sizeof(str)))) {
        fatalException(FN, "Allocate failed for: output_str (malloc)");
    }

    if(!(output_str->seq = malloc(seqLen(seq_in) + 1))) {
        free(output_str);
        fatalException(FN, "Allocate failed for: output_str->seq (malloc)");
    }
...
growString(str* str_in, size_t new_len) {
    ...
    if(!(new_seq = realloc(str_in->seq, new_len + 1))) {
        fatalException(FN, "Allocate failed for: new_seq (realloc)");
    }
    str_in->len = new_len;
    str_in->seq = new_seq;
