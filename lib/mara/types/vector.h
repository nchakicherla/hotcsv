...
removeFromVector(vec* vec_in, size_t pos) {
    ...
    if(!(temp_ptrs = malloc((vec_in->len - 1) * sizeof(void *)))) {
        fatalException(FN, "Allocate failed for: temp_ptrs (malloc)");
    }
...
vectorInitV(int n, ...) {
    ...
    if(!(vec_out = malloc(sizeof(vec)))) {
        fatalException(FN, "Allocate failed for: vec_out (malloc)");
    }
    ...
    if(n != 0) {
        if(!(vec_out->ptrs = malloc(n * sizeof(void *)))) {
            free(vec_out);
            fatalException(FN, "Allocate failed for: vec_out->ptrs (malloc)");
        }
        va_list obj_args;
        va_start(obj_args, n);
...
growVectorPointers(vec* vec_in, size_t new_len) {
    ...
    if(!(new_ptrs = realloc(vec_in->ptrs, new_len * sizeof(void *)))) {
        fatalException(FN, "Allocate failed for: new_ptrs (realloc). Input vecay unchanged");
    }
...
truncateVector(vec* vec_in, size_t new_len) {
    ...
    if(!(temp_ptrs = malloc(new_len * sizeof(void*)))) {
        fatalException(FN, "Allocate failed for: temp_ptrs (malloc). Input vecay unchanged");
    }
