...
matrixInit(int data_type_in, size_t rows, size_t columns, const char* options) {
	...
	if(	data_type_in == 0 ||
		rows == 0 ||
		columns == 0) {
			ERROR(ARG_ERR, FN, "Input argument(s) invalid: data_type_in, rows, and columns must be nonzero");
			return NULL;
	}

	if(data_type_in == FLT_TYPE) { // check if dimensions are too large
		if(rows * (columns / 1024.) > 131072) { // 131072 = 1048576 / 8
			ERROR(ARG_ERR, FN, "Input argument(s) invalid: rows * columns exceeds max size for FLT_TYPE (~ 1GB)");
			return NULL;
		}	
	} else {
		if(rows * (columns / 1024.) > 262144) { // 262144 = 1048576 / 4
			ERROR(ARG_ERR, FN, "Input argument(s) invalid: rows * columns exceeds max size for ITR_TYPE (~ 1GB)");
			return NULL;
		}
	}
	...
	if(!(output_mat = malloc(sizeof(mat)))) {
		fatalException(FN, "Allocate failed for: output_mat (malloc)");
	}
	...
	switch(data_type_in) {
		...
		case ITR_TYPE: {
			if(!(output_mat->data = calloc(rows * columns, sizeof(int32_t)))) {
				free(output_mat);
				fatalException(FN, "Allocate failed for: output_mat->data (malloc)");
			}
		...
		default: {
			ERROR(ARG_ERR, FN, "Argument invalid: data_type_in (matrixInit expects ITR_TYPE or FLT_TYPE)");
			free(output_mat);
			return NULL;
		}
	}
...
duplicateMatrix(mat* mat_in) {
	...
	if(mat_in->data == NULL) {
		ERROR(ARG_ERR, FN, "Input object invalid: mat_in->data = NULL");
		return NULL;
	}

	mat* mat_out = NULL;
	if(!(mat_out = matrixInit(mat_in->datatype, mat_in->m, mat_in->n, NULL))) {
		ERROR(INIT_ERR, FN, "Allocate failed for: mat_out (matrixInit)");
		return NULL;
	}
