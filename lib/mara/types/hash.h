...
duplicateKeyValuePair(kvp* kvp_in) {
    ...
    if(!(kvp_out = malloc(sizeof(kvp)))) {
        fatalException(FN, "Allocate failed for: kvp_out (malloc)");
    }
...
hashInit() {
    ...
    if(!(output_hash = malloc(sizeof(hsh)))) {
        fatalException(FN, "Allocate failed for: output_hash (malloc)");
    }
...
changeKey(hsh* hash_in, const char* key_current, const char* key_new) {
    ...
    if(!(temp_key = seqDupl(key_new))) {
        fatalException(FN, "Allocate failed for temp_key (seqDupl)");
        return 3;
    }
...
addKeyToHash(hsh* hash_in, const char* key_in, void* value_in) {
    ...
    if(!(new_kvp = malloc(sizeof(kvp)))) {
        fatalException(FN, "Allocate failed for: new_kvp (malloc)");
    }
    ...
    if(!(new_kvp->key = malloc(key_length + 1))) {
        free(new_kvp);
        fatalException(FN, "Allocate failed for: new_kvp->key (malloc)");
    }
    ...
    if(hash_in->len == 0) {
        if(!(hash_in->kvptrs = malloc(sizeof(kvp**)))) {
            ...
            fatalException(FN, "Allocate failed for: hash_in->kvptrs (malloc)");
        }
        ...
        if(!(temp_kvps = realloc(hash_in->kvptrs, (hash_in->len + 1) * sizeof(kvp**)))) {
            destroyKeyValuePair(new_kvp);
            fatalException(FN, "Allocate failed for: temp_kvps (realloc)");
        }
...
moveKeyToHash(hsh* hash_in, const char* key_in, void* eating) {
    ...
    if(hash_in->len == 0) {
        if(!(hash_in->kvptrs = malloc(sizeof(kvp**)))) {
            destroyKeyValuePair(new_kvp);
            fatalException(FN, "Allocate failed for: hash_in_cast->kvptrs (malloc)");
        }
        hash_in->kvptrs[0] = new_kvp;
    } else {
        kvp** temp_kvps = NULL;
...
duplicateHash(hsh* hash_in) {
    ...
    if(!(hash_out->kvptrs = malloc(hash_in->len * sizeof(kvp**)))) {
        free(hash_out);
        fatalException(FN, "Allocate failed for: hash_out->kvptrs (malloc)");
    }
